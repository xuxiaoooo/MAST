...
class SequenceDataset(Dataset):
    def __init__(self, data, labels):
        self.data = [torch.tensor(np.array(feature).flatten(), dtype=torch.float32) for feature in data]
    ...
    def __getitem__(self, idx):
        return self.data[idx].T, self.labels[idx]
...
def process_json(json_str):
    if not isinstance(json_str, str):
        return json_str
    data = np.array(json.loads(json_str))
    # 如果列数小于2000，使用0进行填充
...
df = pd.read_csv(f'/Users/xuxiao/WorkBench/AMA_EEG/data/HZ_csv/{band}.csv')
df[['3hz', '6hz', '10hz', '15hz', '24hz', '30hz']] = df[['3hz', '6hz', '10hz', '15hz', '24hz', '30hz']].applymap(process_json)
df['sequence'] = df.apply(lambda row: [row['3hz'], row['6hz'], row['10hz'], row['15hz'], row['24hz'], row['30hz']], axis=1)
df = df[['sequence', 'id']]
...
df = df.merge(pd_label[['简编', '有无自杀史']], left_on='id', right_on='简编', how='left')
# df['诊断'] = df['诊断'].replace({2: 0, 4: 1}) # 将 '诊断' 列的 2 和 4 分别替换为 0 和 1
df = df.rename(columns={'有无自杀史': 'label'})

df = df[['label','sequence']].dropna()
...
train_dataset = SequenceDataset(df_train['sequence'].values, df_train['label'].values)
test_dataset = SequenceDataset(df_test['sequence'].values, df_test['label'].values)
...
for epoch in range(30):
    ...
    with torch.no_grad():
        ...
        for data, labels in test_dataloader:
            ...
            all_probabilities.extend(probabilities[:, 1].detach().numpy().tolist())
...
plt.legend()
plt.savefig(f'/Users/xuxiao/WorkBench/AMA_EEG/code/draw/loss_{band}_{crowd}.jpg', dpi=300, bbox_inches='tight')
# plt.show()
...
plt.savefig(f'/Users/xuxiao/WorkBench/AMA_EEG/code/draw/confusion_matrix_{band}_{crowd}.jpg', dpi=300, bbox_inches='tight')
# plt.show()

# 绘制最后一个 epoch 的 ROC 曲线
with open(f'/Users/xuxiao/WorkBench/AMA_EEG/code/draw/roc_{band}_{crowd}.pkl', 'wb') as f:
